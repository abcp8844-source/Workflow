...
def post_to_facebook():
    ...
    group_url = f"https://graph.facebook.com/v19.0/{GROUP_ID}/feed"
    ...
    try:
        ...
        if "id" in res_data:
            ...
            try:
                group_response = requests.post(group_url, data=payload)
                group_res_data = group_response.json()
                if "id" in group_res_data:
                    print(f"Successfully posted to Facebook Group! Post ID: {group_res_data['id']}")
                else:
                    print(f"Facebook Group API Error: {group_res_data}")
            except Exception as group_err:
                print(f"Error during group posting: {group_err}")
